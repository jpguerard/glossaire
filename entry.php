...
if($_GET['s'])
{
    ...
    $sStartForm .= "<label for=\"lng_source\">"._("SOURCELANGUAGE:")."</label><input id=\"lng_source\" name=\"lng_source\" type=\"text\" value=\"".stripslashes($_GET['s'])."\" /><br />";
...
} elseif($_GET['id'])
{
    ...
    $sStartForm .= "<label for=\"lng_source\">"._("SOURCELANGUAGE:")."</label><input readonly=\"readonly\" id=\"lng_source\" name=\"lng_source\" type=\"text\" value=\"".$oRow->lng_source."\" /> ("._("read only").")<br />";
}
echo "<h2>".ucfirst($sSubmitLabel)." une entrée</h2>\n";
echo $sStartForm;
?>
  <label for="lng_target"><?php echo _("TARGETLANGUAGE:");?></label><input id="lng_target" name="lng_target" type="text" value="<?php echo $oRow->lng_target;?>" /><br />
  <label for="comment"><?php echo _("Comments:");?></label><textarea style="width:18.5em" rows="3" id="comment" name="comment"><?php echo $oRow->comment;?></textarea> (détails, liens vers discussions ou référence)<br />
  <?php
  if($_SESSION['user'])
  {
  ?>
  <label for="source">Source : </label><input readonly="readonly" maxlength="25" id="source" name="source" type="text" value="<?php echo prepare_source($oRow->source);?>" /> (lecture seule)<br />
  <?php
  } else 
  {
  ?>
  <label for="source">Source : </label><input maxlength="25" id="source" name="source" type="text" value="<?php echo $oRow->source?$oRow->source:"*".$_SESSION['admin']."*";?>" /> (par ex. Kde, GNOME, Mozilla, Sun, *<?php echo $_SESSION['admin'];?>*)<br />
  <?php
  }
  ?>
