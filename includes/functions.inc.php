...
{
    global $config;
    $sTableHeader = "<table><tr><th>".$config['lng_source']."</th><th>".$config['lng_target']."</th><th>"._("source")."</th></tr>";
    $sSource = stripslashes(trim($sSource));
    // Google translation
    if($config['at_google'])
    {
        $hFile = fopen("http://translate.google.com/translate_t?langpair=".$config['at_google']."&text=".urlencode($sSource), "r");
        ...
        $sGoogle = trim($aRegs[0])!=$sSource?trim(mb_convert_encoding($aRegs[0],"UTF-8" ,"ISO-8859-1")):"";
        if($sGoogle)
    ...
    {
        $hFile = fopen("http://babelfish.altavista.com/tr?ienc=utf8&lp=".$config['at_altavista']."&trtext=".urlencode($sSource), "r");
        while (!feof($hFile)) $sContents .= fread($hFile, 8192);
        fclose($hFile);
        $aRegs = return_substring($sContents,"<input type=hidden name=\"q\" value=\"","\">");
    
        $sAltavista = trim($aRegs[0])!=$sSource?trim(mb_convert_encoding($aRegs[0],"UTF-8" ,"ISO-8859-1")):"";
        if($sAltavista)
    ...
    if($config['at_amikai'])
    {
        $hFile = fopen("http://standard.beta.amikai.com/amitext/indexUTF8.jsp?langpair=".$config['at_amikai']."&translate=T&sourceText=".urlencode($sSource), "r");
        while (!feof($hFile)) $sContents .= fread($hFile, 8192);
        fclose($hFile);
    ...
    if($sOutput)
    {
        ...
        echo "<p><strong>"._("Warning: automatic translations have to be taken as a mere indication.")."</strong></p>";
        echo $sTableHeader;
        echo $sOutput;
        echo "</table>";
    }
