...
<?php
$sQuery ="SELECT *, DAYOFMONTH(date) as day, MONTH(date) as month, YEAR(date) as year FROM glossary ORDER BY date DESC LIMIT 100";
  ...
  while($oRow = mysql_fetch_object($hResult))
  {
    ...
    if($_SESSION['admin'])
        if($oRow->state != 'deleted')
            ...
            echo "<td><a href=\"entry.php?action=undelete&id=".$oRow->id."\"><img src=\"./images/undelete.png\" alt=\"[restaurer]\" title=\"restaure l'entrée supprimée\" /></a></td>";
    echo "</tr>\n";
  }

  echo "</table>";

} else echo "<h2>Échec de la requête&nbsp;!</h2>" ;
